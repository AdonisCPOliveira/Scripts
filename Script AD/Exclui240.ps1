...
$output_File = "C:\Scripts\Excluir240Dias_$date_file2.txt"
...
#############################################################################################################################################################
#Dias sem logar
$NumberDays = (get-date).AddDays(-240)
Write-Output "-----PESQUISANDO E SALVANDO OS USUÁRIOS ELEGÍVEIS...-----"
$list_user = Get-ADUser -filter {(lastlogondate -le $NumberDays) -AND (enabled -eq $False)} -SearchBase "OU=Desativados,OU=Usuarios,OU=SASCAR,DC=sascar,DC=local" -properties SamAccountName |select SamAccountName
...
#############################################################################################################################################################
#Excluir usuários da OU Desligados, Exporta Dados para Arquivo e Envia Email
Write-Output "-----Excluir usuários da OU Desligados, Exporta Dados para Arquivo e Envia Email - $1...-----"
Remove-ADObject -Identity $16 -Recursive -Confirm:$false
...
$Assunto = "Limpeza 240 Dias"
$Mensagem = "Usuarios excluidos com 240 dias sem utilizacao"
$SMTP = "correio.sascar.com.br"
$Porta = "587"
Send-MailMessage -From $De -To $Para -Subject $Assunto -Body $Mensagem -Attachments C:\Scripts\Excluir240Dias_$date_file2.txt -SmtpServer $SMTP -Port $Porta
