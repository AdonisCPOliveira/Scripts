...
$output_File = "C:\Scripts\Report60Dias_$date_file2.txt"
...
$list_user = Get-ADUser -filter {(lastlogondate -le $NumberDays) -AND (enabled -eq $False)} -SearchBase "DC=sascar,DC=local" -properties SamAccountName | Where-Object {$_.DistinguishedName -notlike "*,$OUDesabilitados"} | select SamAccountName
...
#############################################################################################################################################################
#Exporta Dados para Arquivo e Envia Email
Write-Output "-----Exporta Dados para Arquivo e Envia Email - $1...-----"
Set-ADUser $get_user_info.SamAccountName -Homepage "$10 | 60+: $date"
...
$Porta = "587"
Send-MailMessage -From $De -To $Para -Subject $Assunto -Body $Mensagem -Attachments C:\Scripts\Report60Dias_$date_file2.txt -SmtpServer $SMTP -Port $Porta
