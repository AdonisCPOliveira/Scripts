...
$output_File = "C:\Scripts\Move120Dias_$date_file2.txt"
...
#############################################################################################################################################################
#Dias sem logar
$NumberDays = (get-date).AddDays(-120)
Write-Output "-----PESQUISANDO E SALVANDO OS USUÁRIOS ELEGÍVEIS...-----"
$OUDesabilitados = "OU=Desativados,OU=Usuarios,OU=SASCAR,DC=sascar,DC=local"
$list_user = Get-ADUser -filter {(lastlogondate -le $NumberDays) -AND (enabled -eq $False) -AND (description -notlike "afastamento*")} -SearchBase "DC=sascar,DC=local" -properties SamAccountName | Where-Object {$_.DistinguishedName -notlike "*,$OUDesabilitados"} | select SamAccountName
...
#############################################################################################################################################################
#Migrar usuários para OU Desligados, Exporta Dados para Arquivo e Envia Email
Write-Output "-----Migrar usuários para OU Desligados, Exporta Dados para Arquivo e Envia Email - $1...-----"
Get-ADUser $1 | Move-ADObject -TargetPath "OU=Desativados,OU=Usuarios,OU=SASCAR,DC=sascar,DC=local"
Set-ADUser $get_user_info.SamAccountName -Homepage "$10 | Migrado para OU Desativados em: $date"
$15 | %{Get-ADGroup $_ | Remove-ADGroupMember -Confirm:$false -member $1}
...
$Assunto = "Movimentacao 120 Dias"
$Mensagem = "Usuarios bloqueados e movidos para OU Desabilitados com 120 dias sem utilizacao"
$SMTP = "correio.sascar.com.br"
$Porta = "587"
Send-MailMessage -From $De -To $Para -Subject $Assunto -Body $Mensagem -Attachments C:\Scripts\Move120Dias_$date_file2.txt -SmtpServer $SMTP -Port $Porta
