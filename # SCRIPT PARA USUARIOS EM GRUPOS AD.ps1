...
$list_group = Get-AzureADGroup -SearchString "Administrators" #PROCURA TODOS GRUPOS COM NOME
...
if ($list_group -ne $null) {
    for ($i=0; $i -lt $list_group.Length;$i++){
        ...
        Get-AzureADGroupMember -ObjectId $groupobj -All $true  |  Select DisplayName, UserPrincipalName, department, mail, physicalDeliveryOfficeName, title |
